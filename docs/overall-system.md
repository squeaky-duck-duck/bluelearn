...
- **Method**: A competing *practical* route to the same outcome inside a canonical guide (e.g. a different procedure, toolchain, or technique). Methods live inside their parent guide, each with its own page and URL. Learner **upvotes and downvotes** rank methods against each other; strong sustained preference can promote one to the main guide content.
...
2. **A canonical guide already exists.** The user may:
  ...
  - **Add a method or alternative** under that guide: *methods* for a different practical route to the same outcome, *alternatives* for a different theoretical framing. New methods and alternatives go through the same verifier gate as other new pages.
...
If the reader prefers a different practical route or theoretical framing, they should use votes on the relevant **method or alternative** pages (or author one if it does not exist yet), not use the canonical-guide downvote rubric as a stand-in for approach preference.
